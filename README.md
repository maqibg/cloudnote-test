...
仓库使用 `wrangler.jsonc` 作为唯一配置入口，并默认走本地安装的
`npx wrangler` 工作流。
首次 `npm run deploy` 会自动创建 KV / R2 / D1 资源，并把资源信息写回
`wrangler.jsonc`。
...
cd cloudnote-cf
git sparse-checkout set src schema.sql wrangler.jsonc package.json tsconfig.json deploy.sh deploy.bat .dev.vars.example
```
...
CF Workers 版本说明：
- 生产环境中的敏感信息通过 `wrangler secret put` 写入。
- 本地开发使用 `.dev.vars`，可从 `.dev.vars.example` 复制后填写。
...
# 初始化本地 D1 schema
npx wrangler d1 execute cloudnote-db --local --file=./schema.sql --yes
...
⭐ 如果这个项目对你有帮助，请给个 Star。自动构建触发测试更新。
