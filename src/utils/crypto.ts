...
const HASH_PATTERN = /^[0-9a-f]+:[0-9a-f]+$/i;

function isDerivedHash(hash: string): boolean {
...
export async function hashPassword(password: string): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    crypto.pbkdf2(password, salt, ITERATIONS, KEY_LENGTH, 'sha256', (err: Error | null, derivedKey: Buffer) => {
      if (err) reject(err);
...
export async function verifyPassword(password: string, hash: string): Promise<boolean> {
  if (!isDerivedHash(hash)) {
    return false;
  }

  const [salt, key] = hash.split(HASH_DELIMITER);
  const saltBuffer = Buffer.from(salt, 'hex');
  
  return new Promise((resolve, reject) => {
    crypto.pbkdf2(password, saltBuffer, ITERATIONS, KEY_LENGTH, 'sha256', (err: Error | null, derivedKey: Buffer) => {
      if (err) reject(err);
